...
const Trash = () => {
  ...
  const columns: Column<ArticleData>[] = [
      ...
      accessor: (article) => (
        <div className="flex items-center gap-4 py-1">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="ContentSMedium text-gray-400 mt-0.5 line-clamp-1 max-w-full italic">{article.description}</span>
          </div>
        </div>
      ),
    ...
    },
    {
      header: "Deleted By",
      accessor: (article) => {
        const deletedById = article.deletedBy?.id || article.deletedBy?._id;
        const isMe = deletedById === user?.id;
        return (
          <div className="flex flex-col">
            <span className="ContentSMedium text-gray-400">
              {isMe ? 'Me' : (article.deletedBy?.name || 'Unknown')}
            </span>
            <span className="text-[10px] text-gray-400">
              {isMe ? '' : (article.deletedBy?.email || 'N/A')}
            </span>
          </div>
        );
      }
