...
const Articles = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    ArticleData['status'] | 'ALL' | 'DELETED'
  >('ALL');
  ...
  const [actionType, setActionType] = useState<
    'status' | 'delete' | 'restore' | 'permanent' | null
  >(null);
  const [targetStatus, setTargetStatus] = useState<
    ArticleData['status'] | null
  >(null);
  ...
  const statusMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      if (msg.includes('thu hồi') || msg.includes('ARTICLE_REVOKED')) {
        alert(
          'Bài viết này đã được Admin thu hồi hoặc đang ở trạng thái Nháp. Hệ thống sẽ cập nhật lại danh sách.'
        );
        queryClient.invalidateQueries({ queryKey: ['articles'] });
      } else {
      ...
      accessor: (article) => (
        <div className="flex items-center gap-4 py-1">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="ContentSMedium text-gray-400 mt-0.5 line-clamp-1 max-w-full">
              {article.description}
            </span>
          </div>
        </div>
          ...
          >
            {[
              ...
              <button
                ...
                onClick={() => {
                  setStatusFilter(
                    status as ArticleData['status'] | 'ALL' | 'DELETED'
                  );
                  document
                    .getElementById('status-filter-menu-article')
    ...
    },
    {
      ...
      accessor: (article) => (
        <div className="flex flex-col">
          <span className="ContentSMedium text-gray-700">
            {article.poster.name}
          </span>
          <span className="text-[10px] text-gray-400">
            {article.poster.email}
          </span>
        </div>
      ),
    },
          ...
          {statusFilter === 'DELETED' ? (
            ...
            <>
              <ActionButton
                ...
                onClick={() =>
                  window.open(
                    `${rolePrefix}/article/${article.id}/preview`,
                    '_blank'
                  )
                }
              />
              ...
              {user?.role === 'SUPER_ADMIN' &&
                ['PENDING', 'DRAFT', 'UNPUBLISHED'].includes(
                  article.status
                ) && (
                  <ActionButton
              ...
              {user?.role === 'ADMIN' &&
                user?.permissions?.includes('PUBLISH_TOGGLE') && (
                  <>
                    {article.status === 'PUBLISHED' && (
                      <ActionButton
                        icon={<XCircle className="w-4 h-4" />}
              ...
              {((user?.role === 'SUPER_ADMIN' &&
                ...
                <ActionButton
                  ...
                  tooltip="Edit"
                  onClick={() =>
                    navigate(`${rolePrefix}/article/${article.id}/edit`)
                  }
                />
              )}
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="ContentMRegular text-gray-400 mt-1">
            Manage, review, and publish your content.
          </p>
        </div>
        {(user?.role === 'SUPER_ADMIN' ||
      ...
      <ConfirmModal
        ...
        onConfirm={() =>
          selectedArticle && deleteMutation.mutate(selectedArticle.id)
        }
        title="Move to Trash"
      ...
      <ConfirmModal
        ...
        onClose={handleCloseModals}
        onConfirm={() =>
          selectedArticle && restoreMutation.mutate(selectedArticle.id)
        }
      ...
      <ConfirmModal
        ...
        onClose={handleCloseModals}
        onConfirm={() =>
          selectedArticle && permanentDeleteMutation.mutate(selectedArticle.id)
        }
