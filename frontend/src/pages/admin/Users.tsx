...
const Users = () => {
  ...
  const { 
    ...
    queryKey: ['users', { search: debouncedSearch, status: statusFilter, sort: sortOrder }],
    queryFn: ({ pageParam = 1 }) => 
      adminService.getUsers(debouncedSearch, statusFilter === 'ALL' ? undefined : statusFilter, sortOrder, pageParam as number),
      ...
      accessor: (user) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            <span className="ContentMMedium text-gray-900 truncate">{user.name}</span>
    ...
    },
    {
      ...
      accessor: (user) => (
        <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
          <ActionButton 
            icon={user.status === 'ACTIVE' ? <PowerOff className="w-4 h-4" /> : <Power className="w-4 h-4" />}
            variant={user.status === 'ACTIVE' ? 'warning' : 'success'}
            tooltip={user.status === 'ACTIVE' ? 'Deactivate User' : 'Activate User'}
            onClick={() => { setSelectedUser(user); setActionType('status'); }}
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="ContentMRegular text-gray-400 mt-1">Manage and monitor system administrators and entities.</p>
        </div>
        <Button 
          variant="primary" 
          size="md" 
      ...
      <ConfirmModal
        ...
        onConfirm={() => selectedUser && statusMutation.mutate({ 
          id: selectedUser.id, 
        ...
        message={
          <>
            Are you sure you want to {selectedUser?.status === 'ACTIVE' ? "deactivate" : "activate"} user 
            <span className="font-bold text-gray-900 mx-1">{selectedUser?.name}</span>?
          </>
        }
